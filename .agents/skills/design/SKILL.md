...
### Banner: Generate Visuals (Model Selection)

| Use Case | Model | Quality |
|----------|-------|---------|
| Backgrounds, gradients, patterns | `gemini-2.5-flash-image` | 2K, fast |
| Hero illustrations, product shots | `gemini-3-pro-image-preview` | 4K, detailed |
| Photorealistic scenes, complex art | `gemini-3-pro-image-preview` | 4K, best |
| Quick iterations, A/B variants | `gemini-2.5-flash-image` | 2K, fast |

```bash
python .claude/skills/ai-multimodal/scripts/gemini_batch_process.py \
  --task generate --model gemini-2.5-flash-image \
  --prompt "<banner visual prompt>" --aspect-ratio <platform-ratio> --size 2K --output assets/banners/
```

Pro model prompt tips: be descriptive (style, lighting, mood, composition, palette); specify art direction ("minimalist flat design", "editorial photography"); specify no-text ("no text, no letters, no words" — text overlaid in HTML step).

### Banner: Export to PNG

Serve HTML via local server, then screenshot at exact dimensions:
...
**Output path convention:** `assets/banners/{campaign}/` → `{style}-{width}x{height}.png` (kebab-case). Date prefix for time-sensitive campaigns: `{YYMMDD}-{style}-{size}.png`.
