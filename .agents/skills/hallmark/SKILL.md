...
**Absorbed modules (formerly standalone skills — load on the matching trigger only):**
- [`design-taste-frontend.md`](references/design-taste-frontend.md) — the Three-Dials configuration (DESIGN_VARIANCE / MOTION_INTENSITY / VISUAL_DENSITY) and brief→design-system mapping. Load when the brief needs a real design system pick (Fluent, Carbon, Polaris, Primer, govuk, USWDS, shadcn/ui) or when dial-based tuning helps.
- [`gpt-taste.md`](references/gpt-taste.md) — Awwwards-level GSAP choreography module: Python-driven layout randomization, AIDA page structure, gapless bento grids, pinned/horizontal GSAP scroll paradigms. Load when the brief demands heavy GSAP motion (pinning, stacking, scrubbing) beyond Hallmark's standard motion palette.
- [`high-end-visual-design.md`](references/high-end-visual-design.md) — "expensive" agency patterns: Double-Bezel nested architecture, Island CTA buttons, magnetic hover physics, cinematic entry choreography. Load for premium consumer / Apple-esque / luxury-tier builds.
- [`redesign-existing-projects.md`](references/redesign-existing-projects.md) — comprehensive design audit checklist + fix priority for upgrading existing codebases. Complements `verbs/redesign.md`; load when the user wants a full pattern audit (typography, color, layout, states, content, iconography, code quality).
- [`stitch-design-taste.md`](references/stitch-design-taste.md) — Google Stitch DESIGN.md generator. Load only when the output target is a semantic `DESIGN.md` spec for Stitch screen generation, not a code build.
