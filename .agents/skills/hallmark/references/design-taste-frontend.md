# Design Taste Frontend (formerly `design-taste-frontend`)

Anti-slop frontend module for landing pages, portfolios, and redesigns. The agent reads the brief, infers the right design direction, and ships interfaces that do not look templated. Real design systems when applicable, audit-first on redesigns, strict pre-flight check. Complements Hallmark's Design flow with its Three-Dials configuration and brief→design-system mapping.
