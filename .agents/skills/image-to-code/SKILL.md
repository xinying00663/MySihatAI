...
## 0. GENERATION-PHASE MODULE

Before generating the section reference images, load [`references/image-direction.md`](references/image-direction.md) — the image-direction engine (per-section horizontal output rule, composition anchors, background modes, CTA variation, hero scales, narrative spine, multi-image consistency). Use it to drive the generation phase; return here for analysis and implementation.
